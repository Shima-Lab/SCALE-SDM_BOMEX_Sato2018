...
!!$    real(RP) :: exnr_crs(KA,IA,JA) ! Exner function
    ...
    !--- Temporary added for ascii output of SD to fix the 
    !--- descrepancy of the time (2018/2/17 Y.Sato)
    integer, save :: counter_ascii = 0, interval
    logical, save :: first_ascii = .true.
    !---------------------------------------------------------------------------

    !--- Temporary added for ascii output of SD to fix the 
    !--- descrepancy of the time (2018/2/17 Y.Sato)
    if( (mod(sdm_dmpvar,10)==1) .and. sdm_dmpitva>0.0_RP ) then
      if( first_ascii ) then
        if( sdm_dmpitva < dt ) then
          write(*,*) "sdm_dmpitva should be smalled than TIME_DTSEC_ATMOS_PHY_MP"
          call PRC_MPIstop
        endif
!        if( mod( dt,sdm_dmpitva ) /= 0 ) then
!          write(*,*) "sdm_dmpitva should be multiple of TIME_DTSEC_ATMOS_PHY_MP"
!          call PRC_MPIstop
!        endif
        interval = sdm_dmpitva/dt
        counter_ascii = 0
        if( IO_L ) write(IO_FID_LOG,*) '*** S.D. ASCII Output interval is ', interval
        first_ascii = .false.
      endif
      counter_ascii = counter_ascii + 1
    endif
...
!                      jcb,                                &
    ...
    if( (mod(sdm_dmpvar,10)==1) .and. sdm_dmpitva>0.0_RP .and. interval == counter_ascii ) then
...
!             int(1.E+3_RP*(sdm_dmpitva+0.00010_RP))) == 0 ) then
       if( IO_L ) write(IO_FID_LOG,*) ' *** Output Super Droplet Data in ASCII', counter_ascii
       counter_ascii = 0
...
!!$    call HIST_in( rhoa_sdm(:,:,:), 'RAERO', 'aerosol mass conc.', 'kg/m3', dt)
    ...
    call HIST_in( prr_crs(:,:,2), 'PREC_acc', 'accumulated surface precipitation', 'kg/m2', dt)
