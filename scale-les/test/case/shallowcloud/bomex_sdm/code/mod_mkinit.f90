...
module mod_mkinit
  ...
  integer, public, parameter :: I_BOMEX         = 19
...
!write(*,*)'chk12'
  ...
  subroutine MKINIT_BOMEX
    ...
    if( IO_L ) write(IO_FID_LOG,*)
    if( IO_L ) write(IO_FID_LOG,*) '+++ Module[BOMEX]/Categ[MKINIT]'
    ...
    if( IO_L ) write(IO_FID_LOG,nml=PARAM_MKINIT_BOMEX)
    ...
    do i = IS, IE
    do k = KS, KE
      if( CZ(k) <= 1600.0_RP ) then
       RHOT(k,i,j) = ( pott(k,i,j)+2.0_RP*( rndm(k,i,j)-0.5_RP )*PERTURB_AMP_PT ) * DENS(k,i,j)
      else
       RHOT(k,i,j) = ( pott(k,i,j)+2.0_RP*( rndm(k,i,j)-0.5_RP )*0.0_RP ) * DENS(k,i,j)
      endif
    enddo
    enddo
    enddo
    ...
    if ( flg_bin ) then
       ...
       do k = KS, KE
        if( CZ(k) <= 1600.0_RP ) then
          ...
          QTRC(k,i,j,I_QV) = qv(k,i,j) + 2.0_RP * ( rndm(k,i,j)-0.50_RP ) * 0.0_RP
          QTRC(k,i,j,I_QC) = qc(k,i,j)
        endif 
       enddo
       enddo
